...
{
	char_mp_score = custom_functions.getMPScore(trait_relax.trait_trees["0"], &trait_relax.trait_dataset, FALSE);
	trait_relax.null.mu = char_mp_score;
	trait_relax.null.pi_0 = 0.5;
...
{	
	...
	model.generic.AddGlobal (trait_relax.reference, trait_dependent_site_proportion, terms.trait_relax.p); 
	model.generic.AddGlobal (trait_relax.test, trait_dependent_site_proportion, terms.trait_relax.p); 
...
{	
	...
	if (trait_relax.alternative_starting_point == "RELAX alternative") {
		...
		trait_relax.alternative_model.initial_fit = custom_functions.FitLF_FixedBLs (trait_relax.filter_names, trait_relax.trees, None, { "0" : parsimonious_model_map}, 
																		trait_relax.final_partitioned_mg_results, trait_relax.model_object_map, 
																		{terms.run_options.proportional_branch_length_scaler: trait_relax.scaler});
 ...
 */
lfunction trait_relax.set_character_grid(character_data_filter, tree, mu_interval, pi_0_interval) 
{	
	character_grid = {};
	character_grid["mu"] = {};
	character_grid["pi_0"] = {};
	...
	pi_0_lower_limit = 0.05; // setting above 0 to avoid having 1 being a absorbing state in the character transition matrix
	pi_0_upper_limit = 0.95; // setting below 1 to avoid having 0 being a absorbing state in the character transition matrix
	pi_0_counter = 0;
	for (pi_0=pi_0_lower_limit; pi_0 <=pi_0_upper_limit; pi_0 += pi_0_interval) {
...
{	
	...
	character_grid = trait_relax.set_character_grid(&trait_relax.trait_dataset, trait_relax.trees["0"], mu_interval, pi_0_interval);
	...
	while  (curr_log_likelihood > prev_log_likelihood + min_log_likelihood_diff) {
		...
		for (m=0; m<Abs(character_grid["mu"]); m+=1) {
			for (p=0; p<Abs(character_grid["pi_0"]); p+=1) {
				...
				results = trait_relax.compute_joint_likelihood((character_grid["mu"])[m], (character_grid["pi_0"])[p], &trait_likelihood_function, trait_relax.trait_filter_names, trait_relax.trait_trees, &trait_relax.codon_data, trait_relax.trees, num_of_histories, trait_relax.approximation_method,
...
function trait_relax.optimize_by_brent() 
{
	...
	while  (curr_log_likelihood > prev_log_likelihood + min_log_likelihood_diff) {
		...
		func_args = {0: intial_mu, 2: &trait_likelihood_function, 3: trait_relax.trait_filter_names, 4: trait_relax.trait_trees, 5: &trait_relax.codon_data, 6: trait_relax.trees, 7: num_of_histories, 8: trait_relax.approximation_method, 9: trait_relax.model_object_map, 10: &trait_relax.alternative_model.initial_parameters, 11: FALSE, 12: maxSimulationsNum, 13: None, 14: &base_model_map}; 
		pi_0_lowr_bound = almostZero;
		pi_0_upper_bound = 1-almostZero;
		optimal_pi_0 = pi_0_init;
		log_likelihood = custom_functions.brent_optimize(pi_0_lowr_bound, initial_pi_0, pi_0_upper_bound, "trait_relax.wrapper_compute_joint_likelihood", 1, &func_args, min_dist, &optimal_pi_0);

		// fix pi_0 to the brent optimized one, and optimize the joint likelihood function with regards to pi_0 
		func_args = {1: optimal_pi_0, 2: &trait_likelihood_function, 3: trait_relax.trait_filter_names, 4: trait_relax.trait_trees, 5: &trait_relax.codon_data, 6: trait_relax.trees, 7: num_of_histories, 8: trait_relax.approximation_method, 9: trait_relax.model_object_map, 10: &trait_relax.alternative_model.initial_parameters, 11: FALSE, 12: maxSimulationsNum, 13: None, 14: &base_model_map}; 
		mu_lowr_bound = 0.5*char_mp_score; 
		mu_upper_bound = 2*char_mp_score;
		mu_init = trait_relax.null.trait_mles[0][0]; 
		optimal_mu = mu_init;
		log_likelihood = custom_functions.brent_optimize(mu_lowr_bound, mu_init, mu_upper_bound, "trait_relax.wrapper_compute_joint_likelihood", 0, &func_args, 0.01, &optimal_mu);
		...
		// optimize the sequence model given the best character parameters
		results = trait_relax.compute_joint_likelihood(optimal_mu, optimal_pi_0, &trait_likelihood_function, trait_relax.trait_filter_names, trait_relax.trait_trees, &trait_relax.codon_data, trait_relax.trees, num_of_histories, trait_relax.approximation_method, trait_relax.model_object_map, &trait_relax.alternative_model.initial_parameters, TRUE, maxSimulationsNum, {"write_histories": TRUE, "histories_dir": histories_dir, "return_extended_form": TRUE}, &base_model_map,);
...
lfunction trait_relax.wrapper_compute_joint_likelihood(args_dict) 
{
	...
	trait_tree = (^args_dict)[4];
	sequence_dataset = (^args_dict)[5];
	sequence_trees = (^args_dict)[6];
	histories_num = (^args_dict)[7];
	approximation_method = (^args_dict)[8];
	model_object_map = (^args_dict)[9];
	relax_initial_parameters = (^args_dict)[10];
	optimize_sequence_parameters = (^args_dict)[11];
	max_simulations_num = (^args_dict)[12];
	run_options = (^args_dict)[13];
	base_model_map = (^args_dict)[14];
	
	log_likelihood = trait_relax.compute_joint_likelihood(mu, pi_0, trait_lf_id, trait_filter_name, trait_tree, sequence_dataset, sequence_trees, histories_num, approximation_method, model_object_map, relax_initial_parameters, optimize_sequence_parameters, max_simulations_num, run_options, base_model_map);
 ...
 */
lfunction trait_relax.compute_joint_likelihood(mu, pi_0, trait_lf_id, trait_filter_name, trait_tree, sequence_dataset, sequence_trees, histories_num, approximation_method, model_object_map, relax_initial_parameters, optimize_sequence_parameters, max_simulations_num, run_options, base_model_map) 
{
	...
	utility.ExecuteInGlobalNamespace("trait_relax.trait.model.pi_0 = " + pi_0 + ";");
	...
	for (h = 0; h < histories_num; h = h+1) {  
		...
		history = custom_functions.generate_history(h, &histories_durations_dict, ^trait_lf_id, trait_tree["0"], pi_0, mu, max_simulations_num, run_options);
	...
	for (i=0; i<sites_number; i+=1) {
		...
		for (j=1; j<=histories_num; j+=1) {
			dependent_case_likelihood = dependent_case_likelihood + sitewise_blockwise_likelihoods[0][i*histories_num+j]; // sitewise_blockwise_likelihoods is ordered by sites and then by blocks (=trees)
																													   // for example: say I have two sites 