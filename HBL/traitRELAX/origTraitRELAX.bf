...
global expectedCharSubstitutions = 1.0;                          // set the initial value of mu
AUTOMATICALLY_CONVERT_BRANCH_LENGTHS = 1;  
ACCEPT_ROOTED_TREES   = TRUE; 
RateMatrix = {{*,expectedCharSubstitutions*resFactor}                                  
			  {expectedCharSubstitutions*resFactor,*}};
...
for (this_rep = 0; this_rep < numOfHistories; this_rep = this_rep+1) {    
	...
	//fprintf(MESSAGE_LOG, "labelledTreeStr: ", labelledTreeStr, "\n\n"); // debug!
}
...
function sampleMutationsGivenAncestralsPerBranch(branchesHistory, branchesTransitionsCounter, sonName, fatherState, sonState, branch_length)
{
	...
	for (i=0; i<maxSimulationsNum;i=i+1) {                               // do not exceed the acceptable number of transitions along a branch
		...
		while (disFromFather + timeTillChange < branch_length) {
			...
			// after this section curState is sampled!
		...
		if (curState == sonState) {                                           // if the current state is the son's state -> the simulation succeeded -> record the last jump
			data = {2,1};
			data[0] = branch_length - disFromFather;
			data[1] = curState;
...
function generateHistory (charModel_LF, origTree)
{
	...
	labelledTreeStr = generateTree(origTree, leavesNum, nodesNum, NodeIDtoLeftMostLeafID, branchesHistory, branchesTransitionsCounter, branchNames);
	
	return labelledTreeStr;
}


// 4.7.17 - BUG IN STRING FORMATTING IF NODE NAME IS NESTED WITHIN ANOTHER
/* ____________________________________________________________________	*/
/*	write the history as a tree and return a newick string              */
function generateTree(tree, leavesNumber, nodesNumber, nodeIdToLeftmostChildId, branchToHistory, branchToTransitionsNum, branchToName)
{
	GetString(TreeStr, tree, 0);      // get a string of the tree
	internalsCounter = 0;             // counter of added internal nodes (required for unique naming)
	for (nodeId=0; nodeId<nodesNumber-1; nodeId=nodeId+1) {  // replace each node in the tree string by the string representing the path along the branch corresponding to it
		                              // the last branch is the demme branch exiting from the root, so there is no need to reformat it
		...
		for (j=0; j<branchTransitionsNumber; j=j+1) {
			...
			data = branchHistory[branchTransitionsNumber-j-1]; // branch history is recorded from the father node to its son node.
			                                                   // the transitions are written to the tree from the son to the father
															   // therefore, the order of transitions in both cases in reversed
			branchLength = data[0];
			...
			if (classification == 1) {              // if the branch state is 0 -> label as 'Donor' ("R set")
				lastNode = prefix + "{Donor}";
			} else {                                // if the branch state is 1 -> label as 'Recipient' ("T set")
				lastNode = prefix + "{Recipient}";
			}
			
			// add the sub-string to the string representing the transitions path
			if (nodeStr % "") {                    // handle the special case of the first transition
				nodeStr = lastNode + ":" + branchLength;
			} else {
				if (nodeId < leavesNumber) {       // if the node is a leaf
					nodeStr = "(" + nodeStr + ")" + lastNode + ":" + branchLength;
				} else {                                         // if the node is an internal node -> save the "(" marks to be right to its rightmost child
					nodeStr = nodeStr + ")" + lastNode + ":" + branchLength;
				}
			}
		...
		// replace the node in the original tree with the transitions path starting at it
		TreeStr = strReplace(TreeStr, nodeName, nodeStr); // ASK ELI HOW TO DEAL WITH THE BUG - problem if the node's name is nested in another
		if (isLeaf == 0) {     // if the node is not the rightmost child of itself, the opening "(" should appear right to its rightmost child and not right to itself
			childPrefix = branchToName[nodeIdToLeftmostChildId[nodeId]]; // bug here: this runs over the settings of the child's assignment
			for (r=0; r<branchTransitionsNumber-1; r=r+1) {
				childPrefix = "(" + childPrefix;
			}
			leastMostChildName = branchToName[nodeIdToLeftmostChildId[nodeId]];
			TreeStr = strReplace(TreeStr, leastMostChildName, childPrefix);
		}
	}
	return TreeStr;
} 
