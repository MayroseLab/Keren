 ...
 */
lfunction custom_functions.get_args_str(varying_arg, varying_arg_index, const_args_dict) { // BUG HERE
...
//  based on estimators.FitLF work
// data filter (passe as argument to FitLF) is a dictionary holding a variable which was created in a global namespace... so maybe I SHOULD pass the names and not the variables....
...
lfunction custom_functions.brent_optimize(x_lower, x_start, x_upper, func_id, x_arg_index, func_args,  min_dist, x_max) {
	...
	for (iter=1;iter<=IterationsLimit;iter+=1) { 									  // as long as the number of iteration has not exceeded limit IterationsLimit -> continue
		...
		fprintf(stdout, "\n#### new brant iteration ####\n"); // debug
		/* fprintf(stdout, "iter: ", iter, "\n"); // debug
		fprintf(stdout, "fx: ", fx, "\n"); // debug
		fprintf(stdout, "x: ", x, "\n"); // debug
		fprintf(stdout, "x_max: ", ^ x_max, "\n"); // debug */
...
lfunction custom_functions.DeleteLikelihoodFunction_FixedBLs(lfId) {
	...
	custom_functions.clear_local_constraints(lfId);
...
lfunction custom_functions.FitLF_FixedBLs (data_filter, tree, lf_formula, model_map, initial_values, model_objects, run_options) {
    ...
    if (run_options[utility.getGlobalValue("terms.run_options.retain_lf_object")]) {
        ...
        custom_functions.DeleteLikelihoodFunction_FixedBLs(&likelihoodFunction); // the dictionaries of the models still exist after this action - but it appears that the models are gone. this is opposed to one happens in ComputeLF_FixedBLs
		//DeleteObject(&likelihoodFunction); // switching the above call with this call causes the reference model to not be deleted - but I still get a segmentation fault in the next brent iteration 
    }
...
lfunction custom_functions.ComputeLF_FixedBLs (data_filter, tree, lf_formula, model_map, initial_values, model_objects, run_options) {
    ...
    if (run_options[utility.getGlobalValue("terms.run_options.retain_lf_object")]) {
        ...
        custom_functions.DeleteLikelihoodFunction_FixedBLs(&likelihoodFunction); // can only be deleted if the lf_compomnents contain the same instance just once
    }
    return results;
}
