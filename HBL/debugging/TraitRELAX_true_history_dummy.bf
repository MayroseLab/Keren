...
function trait_relax.process_input() {
	...
	trait_relax.tree_partition = io.SelectAnOption ({
										{"true history", "[Default] use the parittion based as the ture history in the null model"}
										{"maximum parsimony", "use a maximum parsimony based partition in the null model"}
									}, " null model tree partition");
	...
	namespace trait_relax { 
		...
		if (tree_partition == "maximum parsimony") { 
			...
			load_input ({utility.getGlobalValue("terms.prefix"): "trait_relax", utility.getGlobalValue("terms.settings") : {utility.getGlobalValue("terms.settings.branch_selector") : "trait_relax.set_true_history_partition"}}, json_path, seq_data_path, tree_path, trait_data_path); // this function also validates that the species in the tree are exactly the same as the ones in the alignment
		}
	}
...
function trait_relax.do_perliminary_fitting() { 
	...
	namespace trait_relax {
		...
		// keren: 31.01.2018 - try without constraining branch lengths to achieve better initial omegas
...
function trait_relax.optimize_null_model() {
	...
	model_map = {"trait_relax.test" : utility.Filter (trait_relax.selected_branches[0], '_value_', '_value_ == trait_relax.test_branches_name'),
	...
	parameters.SetConstraint (model.generic.GetGlobalParameter (trait_relax.test , terms.trait_relax.k), terms.parameters.one, terms.global);
	
	// fit the RELAX null model to the data - here on dummy data memory capacity is 173m (same as RELAX)
	trait_relax.null_model.fit = custom_functions.FitLF_FixedBLs (trait_relax.filter_names, trait_relax.trees, None, { "0" : model_map}, 
	...
	fprintf(stdout, "null model sequence likelihood: ", trait_relax.null_model.fit.sequence_log_likelihood, "\n"); // debug
	...
	fprintf(stdout, "character+ sequence log likelihood: ", trait_relax.null_model.fit[utility.getGlobalValue("terms.fit.log_likelihood")], "\n"); // debug
	
	fprintf("/groups/itay_mayrose/halabikeren/myScripts/HBL/my_data/dummy_test/debug", "trait_relax.null_model.fit:\n", trait_relax.null_model.fit, "\n"); // debug
	
	// report null fitting results
	io.ReportProgressMessageMD ("TraitRELAX", "null", "* " + selection.io.report_fit (trait_relax.null_model.fit, 9+2, trait_relax.codon_data_info[terms.data.sample_size])); // added 9 frequencies parameters + 2 character model parameters as xp
	...
	parameters.RemoveConstraint(model.generic.GetGlobalParameter (trait_relax.test , terms.trait_relax.k));
...
function trait_relax.set_starting_point() {
	...
	// if the user chose to set the starting point according to the RELAX null model -> set the starting point to hold the MLEs achieved for the sequence model in the trait relax null model fitting 
		...
		trait_relax.alternative_model.initial_parameters = trait_relax.null_model.fit[utility.getGlobalValue("terms.global")];
	}
}
	...
	while  (curr_log_likelihood > prev_log_likelihood + min_log_likelihood_diff) {
		...
		for (m=0; m<Abs(character_grid["mu"]); m+=1) {
			for (p=0; p<Abs(character_grid["pie_0"]); p+=1) {
				...
				fprintf(stdout, "internal interation: ", m, ", ", p, "\n"); // debug
				...
				trait_relax.model_object_map, &trait_relax.alternative_model.initial_parameters, FALSE, maxSimulationsNum, {"return_extended_form": TRUE}, &parsimonious_model_map);
...
function trait_relax.optimize_by_brent() {
	...
	while  (curr_log_likelihood > prev_log_likelihood + min_log_likelihood_diff) {
		...
		fprintf(stdout, "external iteration\n"); // debug
...
function trait_relax.optimize_by_true_history() {
	...
	// optimize the joint model using a two step procedure until convergence 
	char_mp_score = custom_functions.getMPScore(trait_relax.trait_trees["0"], &trait_relax.trait_dataset, FALSE); // set initial mu to be the maximum parsimony score (i.e, minimial number of transitions)
	trait_relax.alternative.mu = char_mp_score;
	trait_relax.alternative.pie_0 = 0.5; // ASK ITAY: initial value for optimization?
	trait_likelihood_function = trait_relax.generate_character_likelihood_function(trait_relax.alternative.mu, trait_relax.null.alternative, "trait_relax.trait", trait_relax.trait_filter_names, trait_relax.trait_trees);
	Optimize(trait_relax.alternative.trait_mles, trait_likelihood_function); // since the frequencies are customized but not optimized, pie_0 remains he same as the one given as input
	trait_relax.alternative.mu = trait_relax.trait.model.mu_01;
	trait_relax.alternative.pie_0 = trait_relax.alternative.trait_mles[0][0];
	
	// KEREN 10.01.18: issue in trait_relax.null.trait_mles: the MLE of mu is absent from the array. It can be accesed directly for now (it is called trait_relax.trait.model.mu_01)
	...
	//fscanf("/groups/itay_mayrose/halabikeren/myScripts/HBL/my_data/dummy_test/labeled_tree_copy.nwk", "String", treeStr);
	
	// convert the "FG" labeling into "Test" labeling and "BG" to "Reference" labeling
	treeStr = custom_functions.strReplace(treeStr, "{FG}", "{Test}");
	treeStr = custom_functions.strReplace(treeStr, "{BG}", "{Reference}");
	...
	model_maps[1] = trait_relax.generate_history_rules_map(true_history_info);
	
	trait_relax.set_codon_model();

	trait_relax.alternative_model.fit = custom_functions.FitLF_FixedBLs(joint_sequence_filter_names, joint_sequence_trees, lf_formula, model_maps, trait_relax.alternative_model.initial_parameters, trait_relax.model_object_map, {terms.run_options.retain_lf_object: TRUE, terms.run_options.proportional_branch_length_scaler: trait_relax.scaler, "custom_lf_formula": TRUE, "SetInitialValues": TRUE, "changeK": TRUE});
	...
	fprintf(stdout, "alternative model sequence likelihood: ", trait_relax.alternative_model.fit.sequence_log_likelihood, "\n"); // debug
	...
	fprintf(stdout, "character+ sequence log likelihood: ", trait_relax.alternative_model.fit[utility.getGlobalValue("terms.fit.log_likelihood")], "\n"); // debug
	fprintf(stdout, "trait_relax.p: "< trait_relax.p, "\n"); // debug
	
	fprintf("/groups/itay_mayrose/halabikeren/myScripts/HBL/my_data/dummy_test/debug", "trait_relax.alternative_model.fit:\n", trait_relax.alternative_model.fit, "\n"); // debug
...
function trait_relax.document_alternative_fit() {
	...
	selection.io.json_store_lf (trait_relax.json,
								...
								trait_relax.alternative_model.fit.log_likelihood, // the full sequence + character log likelihood 
...
function trait_relax.optimize_alternative_model() {
	...
	number_of_params_to_add = 9+2; // debug the number of parameters to add to the resport be 9 codon frequencies parpameters + 2 character parameters, aside the sequence optimized parameters which are already included in the report
	// apply a restriction according to the user's request
	if (trait_relax.site_propertions_freedom == "No") {
		trait_relax.p := 1;
		number_of_params_to_add = number_of_params_to_add - 1; // if the user chose the non prop version, remove one parameter for not consindering the fixed p as an additional parameter
	}
	...
	// optimize based on the true history 
	trait_relax.optimize_by_true_history();
	
	// report the fitting results - BUG IS HERE! 5.2.18
	io.ReportProgressMessageMD("TraitRELAX", "alt", "* " + selection.io.report_fit (trait_relax.alternative_model.fit, number_of_params_to_add, trait_relax.codon_data_info[terms.data.sample_size])); // added 9 frequencies parameters + 2 character model parameters + 1 prop parameter as xp
...
lfunction trait_relax.compute_joint_likelihood(mu, pie_0, trait_filter_name, trait_tree, sequence_dataset, sequence_trees, histories_num, approximation_method, model_object_map, relax_initial_parameters, optimize_sequence_parameters, max_simulations_num, run_options, base_model_map) {
	...
	if (optimize_sequence_parameters) {
		trait_relax.alternative_model.initial_parameters = custom_functions.FitLF_FixedBLs(joint_sequence_filter_names, joint_sequence_trees, lf_formula, model_maps, ^relax_initial_parameters, model_object_map, {utility.getGlobalValue("terms.run_options.retain_lf_object"): retain_lf, terms.run_options.proportional_branch_length_scaler: trait_relax.scaler, "custom_lf_formula": TRUE, "SetInitialValues": TRUE});
	} 
	
	// generate and compute the log likelihood of the joint likelihood function (i.e, while fixing the sequence model parameters)
	else {	
		trait_relax.alternative_model.initial_parameters = custom_functions.ComputeLF_FixedBLs(joint_sequence_filter_names, joint_sequence_trees, lf_formula, model_maps, ^relax_initial_parameters, model_object_map, {utility.getGlobalValue("terms.run_options.retain_lf_object"): retain_lf, terms.run_options.proportional_branch_length_scaler: trait_relax.scaler, "custom_lf_formula": TRUE, "SetInitialValues": TRUE});
	}
...
function trait_relax.set_true_history_partition(partition_info, trait_data) {
