...
trait_relax.compute_or_optimize = io.SelectAnOption ({
											{"compute", "[Default] call create_compute_delete()"}
											{"optimize", "call call create_optimize_delete()"}
										}, " choose for debugging");

if (trait_relax.compute_or_optimize % "compute") {
	trait_relax.create_compute_delete();
} else {
	trait_relax.create_optimize_delete();
}
...
function trait_relax.create_compute_delete() {
	...
	while (TRUE) { // fails in the second iteration - regardless if it is local or global function (verified)
