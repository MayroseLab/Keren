...
fprintf(stdout, "mu: ", trait_relax.null.mu , "\n");
fprintf(stdout, "pi_0: ", trait_relax.null.pi_0, "\n\n");


/* debug - print the expected number of transitions */
expected_transitions_num = tree_size * trait_relax.null.mu;
expected_01_transitions_num = trait_relax.null.pi_0 * expected_transitions_num;
...
for (hist = 0; hist < histories_num; hist = hist+1) {  
	...
	history_data = custom_functions.generate_history(hist, &histories_durations_dict, trait_likelihood_function, trait_relax.trait_trees[0], trait_relax.null.pi_0, trait_relax.null.mu, max_simulations_num, {"write_histories": TRUE, "histories_dir": histories_dir, "debug_mode": TRUE});
