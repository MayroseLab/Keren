	...
	while  (curr_log_likelihood > prev_log_likelihood + min_log_likelihood_diff) {
		...
		for (m=0; m<Abs(character_grid["mu"]); m+=1) {
			for (p=0; p<Abs(character_grid["pie_0"]); p+=1) {
				...
				trait_relax.model_object_map, &trait_relax.alternative_model.initial_parameters, FALSE, maxSimulationsNum, None, &parsimonious_model_map);
...
lfunction trait_relax.generate_character_likelihood_function(mu, pie_0, lf_prefix, data_filter, tree) {
	...
	// in brent we use here models.binaryML.ModelDescription instead of models.binary.ModelDescription in order to to optimize the frequencies
