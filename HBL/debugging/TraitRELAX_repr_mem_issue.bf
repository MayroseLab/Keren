...
trait_relax.cause_memory_increase();
...
/** 
 * @name trait_relax.cause_men_blowup
 * example of memory increase 
 */
function trait_relax.cause_memory_increase() {
	...
	results = trait_relax.compute_joint_likelihood(1, 0.25, trait_relax.trait_filter_names, trait_relax.trait_trees, &trait_relax.codon_data, trait_relax.trees, 100, "Multiple Histories", trait_relax.model_object_map, None, trait_relax.model_object_map, TRUE, maxSimulationsNum, None, &parsimonious_model_map);
}
...
lfunction trait_relax.compute_joint_likelihood(mu, pie_0, trait_filter_name, trait_tree, sequence_dataset, sequence_trees, histories_num, approximation_method, model_object_map, relax_initial_parameters, sequence_model_objects, optimize_sequence_parameters, max_simulations_num, run_options, base_model_map) {
	...
	if (optimize_sequence_parameters == TRUE) {	
		trait_relax.alternative_model.initial_parameters = custom_functions.FitLF_FixedBLs(joint_sequence_filter_names, joint_sequence_trees, lf_formula, model_maps, None, sequence_model_objects, {terms.run_options.retain_lf_object: FALSE, terms.run_options.proportional_branch_length_scaler: trait_relax.scaler, "custom_lf_formula": TRUE});
	}
