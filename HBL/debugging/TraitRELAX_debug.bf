...
mu_interval = 0.5;		// increased the interval for debugging
pie_0_interval = 0.5;	// increased the interval for debugging
...
function trait_relax.optimize_null_model() {
	...
	parsimonious_model_map = {"trait_relax.test" : utility.Filter (trait_relax.selected_branches[0], '_value_', '_value_ == trait_relax.test_branches_name'),
	...
	trait_relax.null_model.fit = custom_functions.FitLF_FixedBLs (trait_relax.filter_names, trait_relax.trees, None, { "0" : parsimonious_model_map}, 
																		...
																		{utility.getGlobalValue("terms.run_options.retain_lf_object"): FALSE, terms.run_options.proportional_branch_length_scaler: trait_relax.scaler, "custom_lf_formula": FALSE});
...
function trait_relax.set_starting_point() {
	...
	if (trait_relax.alternative_starting_point == "RELAX alternative") {
		...
		trait_relax.alternative_model.initial_parameters = custom_functions.FitLF_FixedBLs (trait_relax.filter_names, trait_relax.trees, None, { "0" : base_model_map}, 
																		trait_relax.final_partitioned_mg_results, trait_relax.model_object_map, 
																		{utility.getGlobalValue("terms.run_options.retain_lf_object"): TRUE, terms.run_options.proportional_branch_length_scaler: trait_relax.scaler, "custom_lf_formula": FALSE});
...
function trait_relax.optimize_by_grid() {
	...
	while  (curr_log_likelihood > prev_log_likelihood + min_log_likelihood_diff) {
		...
		for (m=0; m<Abs(character_grid["mu"]); m+=1) {
			for (p=0; p<Abs(character_grid["pie_0"]); p+=1) {
				...
				trait_relax.model_object_map, &trait_relax.alternative_model.initial_parameters, FALSE, maxSimulationsNum, {"return_extended_form": TRUE, utility.getGlobalValue("terms.run_options.retain_lf_object"): FALSE}, &base_model_map);
		...
		io.ReportProgressMessageMD ("TraitRELAX", "alt", "\n### Optimal character model parameters fixed");
	...
	// update the log likelihood to be the joint character sequence likelihood
...
function trait_relax.optimize_by_brent() {
	...
	while  (curr_log_likelihood > prev_log_likelihood + min_log_likelihood_diff) {
		...
		// setting the model again using trait_relax.set_codon_model() didn't help... still I get an error: 'trait_relax.reference' does not refer to an existing object of type Model from the first attempt to assign a model in this loop in the second iteration
		// however - this does not happen if in the end of the enternal iteration, I call (via trait_relax.compute_joint_likelihood, 11th argument) to custom_functions.ComputeLF_FixedBLs rather than custom_functions.FitLF_FixedBLs - does this suggest that te first function deletes something differently from the latter? a comparison by eye didn't reveal anything of this sort.
		// the same issue occurs in trait_relax.optimize_by_grid(), and it shows the exract responses as trait_relax.optimize_by_brent to the above attempt to resolve it.
		...
		intial_mu = trait_relax.null.mu; // KEREN 10.01.18: for some reason, mu is absent from the MLEs array. Will be resolved in latter varsions.
		...
		fprintf(stdout, "* pie_0 = ", optimal_pie_0, "\n\n"); 
	...
	fprintf(stdout, "trait_relax.alternative_model.fit: ", trait_relax.alternative_model.fit, "\n"); // debug 
...
function trait_relax.optimize_alternative_model() {
	...
	added_params_num = 9 + 2; // added 9 frequencies parameters + 2 character model parameters
	if (trait_relax.site_propertions_freedom == "Yes") {
		added_params_num += 1; // add an additional paameter of the trait depedent site proportion 
	}
	...
	// 5.2.18:keren: the reporting bug (faulty parameters reported in the tables generated by report_dnds) is rooted in the function models.codon.BS_REL.ExtractMixtureDistribution (trait_relax.test) - which attempts to extract the MLEs of the omegas and their proportion from an alreadt deleted model instance - wehich was deleted when the likelihood function of the last optimization cycle was deleted
	...
	trait_relax.inferred_distribution = parameters.GetStickBreakingDistribution (models.codon.BS_REL.ExtractMixtureDistributionFromFit (trait_relax.test, trait_relax.alternative_model.fit)) % 0;
	selection.io.report_dnds (trait_relax.inferred_distribution);

	io.ReportProgressMessageMD("TraitRELAX", "alt", "* The following rate distribution was inferred for branches under character 0");
	trait_relax.inferred_distribution_ref = parameters.GetStickBreakingDistribution (models.codon.BS_REL.ExtractMixtureDistributionFromFit (trait_relax.reference, trait_relax.alternative_model.fit)) % 0;
...
lfunction trait_relax.compute_joint_likelihood(mu, pie_0, trait_filter_name, trait_tree, sequence_dataset, sequence_trees, histories_num, approximation_method, model_object_map, relax_initial_parameters, optimize_sequence_parameters, max_simulations_num, run_options, base_model_map) {
	...
	// create the codono model again, since it is deleted every time a likelihood function is deleted - it really slows down my code:(
