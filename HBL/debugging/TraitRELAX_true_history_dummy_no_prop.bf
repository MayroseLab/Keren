...
function trait_relax.optimize_null_model() {
	...
	// b elongs to the attempt when using Sergei's function																	
	/* trait_relax.null_model.fit = estimators.FitLF (trait_relax.filter_names, trait_relax.trees, { "0" : model_map}, 
																		trait_relax.final_partitioned_mg_results, trait_relax.model_object_map, 
																		{terms.run_options.retain_lf_object: TRUE, terms.run_options.proportional_branch_length_scaler: trait_relax.scaler, "custom_lf_formula": FALSE}); */
																		
	//fprintf(stdout, "[DEBUG] results of null model likelihood optimization:\n", trait_relax.null_model.fit, "\n\n"); // debug
...
function trait_relax.optimize_by_true_history() {
	...
	lf_formula = "'Log(SITE_LIKELIHOOD[0])'";
	...
	joint_sequence_filter_names[0] = "data_filter_1";
	joint_sequence_trees[0] = true_history_info;
	model_maps[0] = trait_relax.generate_history_rules_map(true_history_info);
	
	trait_relax.set_codon_model();
 
	// original traitrelax call: sp:0.5/1 -> mle:1.34
	...
	// call the same as relax: sp:0.5/1 -> mle: 0.82
	// make it exactly the same as relax and check if the values are still crazy 0 values are no crazy anymore. compare FitLF to FitLF_FixedBLs one by one to check what went wrong
	/* trait_relax.alternative_model.fit = estimators.FitLF (trait_relax.filter_names, trait_relax.trees, { "0" : model_map}, trait_relax.final_partitioned_mg_results, trait_relax.model_object_map, {terms.run_options.retain_lf_object: TRUE., "changeK": TRUE}); */
	
	io.ReportProgressMessageMD ("TraitRELAX", "alt", "RELAX parameters optimized");
	
	// extract the sequence likelihood and by site likelihood																
	log_likelihood = trait_relax.alternative_model.fit [utility.getGlobalValue("terms.fit.log_likelihood")];
...
function trait_relax.optimize_alternative_model() {
	...
	io.ReportProgressMessageMD("TraitRELAX", "alt", "* " + selection.io.report_fit (trait_relax.alternative_model.fit, 9+2, trait_relax.codon_data_info[terms.data.sample_size])); // added 9 frequencies parameters + 2 character model parameters as xp
	...
	trait_relax.inferred_distribution = parameters.GetStickBreakingDistribution (models.codon.BS_REL.ExtractMixtureDistribution (trait_relax.test)) % 0; // gives 0 in RELAX as well
	selection.io.report_dnds (trait_relax.inferred_distribution);
	
	io.ReportProgressMessageMD("TraitRELAX", "alt", "* The following rate distribution was inferred for branches under character 0");
	trait_relax.inferred_distribution_ref = parameters.GetStickBreakingDistribution (models.codon.BS_REL.ExtractMixtureDistribution (trait_relax.reference)) % 0; // here, something might be wrong...
...
lfunction trait_relax.compute_joint_likelihood(mu, pie_0, trait_filter_name, trait_tree, sequence_dataset, sequence_trees, histories_num, approximation_method, model_object_map, relax_initial_parameters, optimize_sequence_parameters, max_simulations_num, run_options, base_model_map) {
	...
	for (h = 0; h < histories_num; h = h+1) {  
		...
		if (approximation_method == "Multiple Histories") {
			...
			if (h == 0) {
				lf_formula = lf_formula + "SITE_LIKELIHOOD[" + (h) + "]";
			} else {
				lf_formula = lf_formula + "+SITE_LIKELIHOOD[" + (h) + "]";
			}
			
			// add he history to the sequence dataset 
			joint_sequence_filter_names [h] = "data_filter_" + h+1;
			custom_functions.dup_data_filter(joint_sequence_filter_names [h], sequence_dataset);
			joint_sequence_trees[h] = history;
			model_maps[h] = trait_relax.generate_history_rules_map(history);
		}
	}
	...
	// user chose to average the histories into a single expected history    --> average the histories and use the expected history in the joint likelihood function formula
		...
		lf_formula = lf_formula + "SITE_LIKELIHOOD[0]))'";
	}
...
lfunction trait_relax.set_mp_partition(partition_info, trait_data) {
	...
	//fprintf(stdout, "selection by mp partition output: ", {"0": return_set }, "\n"); // debug
	return {"0": return_set }; 
