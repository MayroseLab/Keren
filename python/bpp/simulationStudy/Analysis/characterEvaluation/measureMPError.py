...
def parse_biopp_history(history_path, base_tree_path):
    ...
    for node in history.traverse("postorder"):
        if node != history.get_tree_root():
            ...
            if len(node.get_children()) > 2:
                ...
                new = history.get_tree_root().add_child(child=None, name=missing_node.name, dist=missing_node.dist,
                                                        support=None)
    ...
    for node in history.traverse("postorder"):
        if "base" in node.name:
            ...
            base_parent = base_tree.search_nodes(name=child.name.rstrip())[0].up
...
def parse_union_tree(history_1, history_2, base_tree_path, debug=False):
    ...
    for original_node in base_tree.traverse(
            "preorder"):  # traverse the tree in pre-order to assure that for any visited node, its parent from the base branch is already in the united tree
        ...
        if original_parent != None:  # will be none only in the case the original node is the root
            ...
            hist_1_parent = history_1.search_nodes(name=original_parent.name.rstrip())[
                0]  # need to check names consistency across the 3 trees
            for child in hist_1_parent.children:
                if len(base_tree.search_nodes(name=child.name)) == 0 and len(child.get_children()) == 1 and len(
                        child.search_nodes(
                                name=original_node.name)) > 0:  # if the child does not exist in the base tree, it represents a mapping node that was created out of breaking a branch in the original tree
            ...
            hist_2_parent = history_2.search_nodes(name=original_parent.name.rstrip())[
                0]  # need to check names consistency across the 3 trees
            ...
            while not hist_1_done or not hist_2_done:
                ...
                if hist_1_curr_child.name == hist_2_curr_child.name and len(
                        base_tree.search_nodes(name=hist_1_curr_child.name)) > 0:
                    break
            ...
            if residual < 0:
                ...
                print("curr_union_parent.get_distance(united_tree.search_nodes(name=original_parent.name.rstrip())[0]): ",
            ...
            curr_union_parent.add_feature("history_1_label",
                                          history_1.search_nodes(name=original_parent.name.rstrip())[0].label)
            curr_union_parent.add_feature("history_2_label",
                                          history_2.search_nodes(name=original_parent.name.rstrip())[0].label)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_dir', '-i',
                        help='directory that holds the the maximum parsimony solution based histories and the troe histories',
                        required=True)
    parser.add_argument('--base_trees_dir', '-t',
                        help='directory to the tree on which character evolution was simulated', required=True)
    parser.add_argument('--output_path', '-o',
                        help='path that will hold a csv file with the distances between the true and mp history per replicate',
                        required=True)
    ...
    df = pd.DataFrame(
        columns=["tree_length", "mu", "#taxa", "k", "replicate", "expected(#transitions)", "simulated(#transitions)",
                 "mp(#transitions)", "distance(true_history,mp_history)", "distance(true_history,expected_history)"])
    ...
    for path in os.listdir(input_dir):
        if "replicate" in path:
            record = {"tree_length": tree_length, "mu": mu, "#taxa": taxa_num, "k": k,
                      "expected(#transitions)": expected_transitions_num}
            full_path = input_dir + path + "/"
            record["replicate"] = int(replicate_regex.search(full_path).group(1))
            print("replicate=", record["replicate"])
            # mu=4, 600 codons, k=1: bug in replicates [0,115] 0 is different bug from the rest
            # mu=16, 600 codons, k-1: bug in replicates [19, 26, 58, 80, 113, 117, 143, 183]
            # mu=2, 600 codons, k=1: bug in replicates [84]
            # mu=8, 600 codons, k=1: bug in replicates [0, 1, 3, 4, 5, 7, 8, 9, 10, 12, 14...]
            if not os.path.exists(base_trees_dir):
                res = os.system("mkdir -p " + base_trees_dir)
            base_tree_path = create_base_tree(full_path + "character_data/true_history.nwk",
                                              base_trees_dir + str(record["replicate"]) + ".nwk")
            ...
            if os.path.exists(full_path + "/traitrelax_result/histories_evaluation/1000_analytic_expected_mapping.nwk"):
                expected_history = parse_biopp_history(
                    full_path + "/traitrelax_result/histories_evaluation/1000_analytic_expected_mapping.nwk",
                    base_tree_path)
            ...
            if expected_history != None:
                record["distance(true_history,expected_history)"] = compute_distance(true_history, expected_history,
                                                                                     base_tree_path)
