...
def extract_traitrelax_parameters(content, hypothesis, dictionary):
    ...
    if hypothesis == "alternative":
        regex_strings["logl"] = "\*\*\*\*\s*Alternative model likelihood after optimization\s*\*\*\*\*.*?Overall Log likelihood\.*\:\s*(-\d*\.?\d*)"
        regex_strings["1_Full.theta"] = "\*\*\*\*\s*Alternative model likelihood after optimization\s*\*\*\*\*.*?\.1_Full.theta_1\.*\:\s*(\d*\.?\d*)"
        regex_strings["1_Full.theta1"] = "\*\*\*\*\s*Alternative model likelihood after optimization\s*\*\*\*\*.*?\.1_Full.theta1_1\.*\:\s*(\d*\.?\d*)"
        regex_strings["1_Full.theta2"] = "\*\*\*\*\s*Alternative model likelihood after optimization\s*\*\*\*\*.*?\.1_Full.theta2_1\.*\:\s*(\d*\.?\d*)"
        regex_strings["2_Full.theta"] = "\*\*\*\*\s*Alternative model likelihood after optimization\s*\*\*\*\*.*?\.2_Full.theta_1\.*\:\s*(\d*\.?\d*)"
        regex_strings["2_Full.theta1"] = "\*\*\*\*\s*Alternative model likelihood after optimization\s*\*\*\*\*.*?\.2_Full.theta1_1\.*\:\s*(\d*\.?\d*)"
        regex_strings["2_Full.theta2"] = "\*\*\*\*\s*Alternative model likelihood after optimization\s*\*\*\*\*.*?\.2_Full.theta2_1\.*\:\s*(\d*\.?\d*)"
        regex_strings["3_Full.theta"] = "\*\*\*\*\s*Alternative model likelihood after optimization\s*\*\*\*\*.*?\.3_Full.theta_1\.*\:\s*(\d*\.?\d*)"
        regex_strings["3_Full.theta1"] = "\*\*\*\*\s*Alternative model likelihood after optimization\s*\*\*\*\*.*?\.3_Full.theta1_1\.*\:\s*(\d*\.?\d*)"
        regex_strings["3_Full.theta2"] = "\*\*\*\*\s*Alternative model likelihood after optimization\s*\*\*\*\*.*?\.3_Full.theta2_1\.*\:\s*(\d*\.?\d*)"
        regex_strings["kappa"] = "\*\*\*\*\s*Alternative model likelihood after optimization\s*\*\*\*\*.*?\.kappa_1\.*\:\s*(\d*\.?\d*)"
        regex_strings["p"] = "\*\*\*\*\s*Alternative model likelihood after optimization\s*\*\*\*\*.*?\.p_1\.*\:\s*(\d*\.?\d*)"
        regex_strings["omega1"] = "\*\*\*\*\s*Alternative model likelihood after optimization\s*\*\*\*\*.*?\.omega1_1\.*\:\s*(\d*\.?\d*)"
        regex_strings["omega2"] = "\*\*\*\*\s*Alternative model likelihood after optimization\s*\*\*\*\*.*?\.omega2_1\.*\:\s*(\d*\.?\d*)"
        regex_strings["theta1"] = "\*\*\*\*\s*Alternative model likelihood after optimization\s*\*\*\*\*.*?\.theta1_1\.*\:\s*(\d*\.?\d*)"
        regex_strings["theta2"] = "\*\*\*\*\s*Alternative model likelihood after optimization\s*\*\*\*\*.*?\.theta2_1\.*\:\s*(\d*\.?\d*)"
        regex_strings["k"] = "\*\*\*\*\s*Alternative model likelihood after optimization\s*\*\*\*\*.*?\.k_2\.*\:\s*(\d*\.?\d*)"
        regex_strings["mu"] = "\*\*\*\*\s*Alternative model likelihood after optimization\s*\*\*\*\*.*?\.mu\.*\:\s*(\d*\.?\d*)"
        regex_strings["pi0"] = "\*\*\*\*\s*Alternative model likelihood after optimization\s*\*\*\*\*.*?\.pi0\.*\:\s*(\d*\.?\d*)"
    else:
        regex_strings["logl"] = "\*\*\*\*\s*Null model fitting\s*\*\*\*\*.*?\*\*\s*Model parameters after sequence model optimizaiton\s*\*\* .*?Overall Log likelihood\.*\:\s*(-\d*\.?\d*)"
        regex_strings["1_Full.theta"] = "\*\*\*\*\s*Null model fitting\s*\*\*\*\*.*?\*\*\s*Model parameters after sequence model optimizaiton\s*\*\*.*?\.1_Full.theta_1\.*\:\s*(\d*\.?\d*)"
        regex_strings["1_Full.theta1"] = "\*\*\*\*\s*Null model fitting\s*\*\*\*\*.*?\*\*\s*Model parameters after sequence model optimizaiton\s*\*\*.*?\.1_Full.theta1_1\.*\:\s*(\d*\.?\d*)"
        regex_strings["1_Full.theta2"] = "\*\*\*\*\s*Null model fitting\s*\*\*\*\*.*?\*\*\s*Model parameters after sequence model optimizaiton\s*\*\*.*?\.1_Full.theta2_1\.*\:\s*(\d*\.?\d*)"
        regex_strings["2_Full.theta"] = "\*\*\*\*\s*Null model fitting\s*\*\*\*\*.*?\*\*\s*Model parameters after sequence model optimizaiton\s*\*\*.*?\.2_Full.theta_1\.*\:\s*(\d*\.?\d*)"
        regex_strings["2_Full.theta1"] = "\*\*\*\*\s*Null model fitting\s*\*\*\*\*.*?\*\*\s*Model parameters after sequence model optimizaiton\s*\*\*.*?\.2_Full.theta1_1\.*\:\s*(\d*\.?\d*)"
        regex_strings["2_Full.theta2"] = "\*\*\*\*\s*Null model fitting\s*\*\*\*\*.*?\*\*\s*Model parameters after sequence model optimizaiton\s*\*\*.*?\.2_Full.theta2_1\.*\:\s*(\d*\.?\d*)"
        regex_strings["3_Full.theta"] = "\*\*\*\*\s*Null model fitting\s*\*\*\*\*.*?\*\*\s*Model parameters after sequence model optimizaiton\s*\*\*.*?\.3_Full.theta_1\.*\:\s*(\d*\.?\d*)"
        regex_strings["3_Full.theta1"] = "\*\*\*\*\s*Null model fitting\s*\*\*\*\*.*?\*\*\s*Model parameters after sequence model optimizaiton\s*\*\*.*?\.3_Full.theta1_1\.*\:\s*(\d*\.?\d*)"
        regex_strings["3_Full.theta2"] = "\*\*\*\*\s*Null model fitting\s*\*\*\*\*.*?\*\*\s*Model parameters after sequence model optimizaiton\s*\*\*.*?\.3_Full.theta2_1\.*\:\s*(\d*\.?\d*)"
        regex_strings["kappa"] = "\*\*\*\*\s*Null model fitting\s*\*\*\*\*.*?\*\*\s*Model parameters after sequence model optimizaiton\s*\*\*.*?model 2.*?\.kappa\.*\:\s*(\d*\.?\d*)"
        regex_strings["p"] = "\*\*\*\*\s*Null model fitting\s*\*\*\*\*.*?\*\*\s*Model parameters after sequence model optimizaiton\s*\*\*.*?model 2.*?\.p\.*\:\s*(\d*\.?\d*)"
        regex_strings["omega1"] = "\*\*\*\*\s*Null model fitting\s*\*\*\*\*.*?\*\*\s*Model parameters after sequence model optimizaiton\s*\*\*.*?model 2.*?\.omega1\.*\:\s*(\d*\.?\d*)"
        regex_strings["omega2"] = "\*\*\*\*\s*Null model fitting\s*\*\*\*\*.*?\*\*\s*Model parameters after sequence model optimizaiton\s*\*\*.*?model 2.*?\.omega2\.*\:\s*(\d*\.?\d*)"
        regex_strings["theta1"] = "\*\*\*\*\s*Null model fitting\s*\*\*\*\*.*?\*\*\s*Model parameters after sequence model optimizaiton\s*\*\*.*?model 2.*?\.theta1\.*\:\s*(\d*\.?\d*)"
        regex_strings["theta2"] = "\*\*\*\*\s*Null model fitting\s*\*\*\*\*.*?\*\*\s*Model parameters after sequence model optimizaiton\s*\*\*.*?model 2.*?\.theta2\.*\:\s*(\d*\.?\d*)"
        regex_strings["k"] = "\*\*\*\*\s*Null model fitting\s*\*\*\*\*.*?\*\*\s*Model parameters after sequence model optimizaiton\s*\*\* .*?model 2.*?\.k\.*\:\s*(\d*\.?\d*)"
        regex_strings["mu"] = "\*\*\*\*\s*Null model fitting\s*\*\*\*\*.*?\*\*\s*Model parameters after sequence model optimizaiton\s*\*\*.*?\.mu\.*\:\s*(\d*\.?\d*)"
        regex_strings["pi0"] = "\*\*\*\*\s*Null model fitting\s*\*\*\*\*.*?\*\*\s*Model parameters after sequence model optimizaiton\s*\*\*.*?\.pi0\.*\:\s*(\d*\.?\d*)"
...
if __name__ == '__main__':
    ...
    if method == "traitrelax" and bootstrap_on_char_only: # simulate only character data, and create parameter files that use the data_path
        cmd = "python /groups/itay_mayrose/halabikeren/myScripts/python/simulator/TraitSimulator.py -o " + simulations_dir +  "data/ -t " + tree_path + " -mu " + inferred_parameters["mu"] + " -pi0 " + inferred_parameters["pi0"] + " -imu " + initial_parameters["mu"] + " -ipi0 " + initial_parameters["pi0"] + " -ikappa " + initial_parameters["kappa"] + " -iomega0 " + initial_parameters["omega0"] + " -iomega1 " + initial_parameters["omega1"] + " -iomega2 " + initial_parameters["omega2"] + " -ip0 " + initial_parameters["p0"] + " -ip1 " + initial_parameters["p1"] + " -ik " + initial_parameters["k"] + " -rep " + str(num_of_replicates) + " -s " + data_path + " -rep " + str(num_of_replicates)
    if method == "traitrelax" and without_trait_simulations:
        cmd = "python /groups/itay_mayrose/halabikeren/myScripts/python/simulator/RELAXSimulator.py -o " + simulations_dir + "data/ -t " + history_path + " -kappa " + \
            ...
            alignment_length) + " -rep " + str(num_of_replicates) + " -n1t " + inferred_parameters[
                  ...
                  "3_Full.theta2"] + " -in3t2 " + initial_parameters["3_Full.theta2"] + " -imu " + initial_parameters["mu"] + " -ipi0 " + initial_parameters["pi0"] + " -cd " + character_data_path
    ...
    if set_advanced_parameters:
        for path in os.listdir(traitrelax_param_path):
            with open(traitrelax_param_path + path, "a") as infile:
                infile.write("\noptimization.character.twice=1")
